...
!       RTO     RATIO USED TEMPORARILY IN MANY PLACES
      ...
      SPFD(KOMX(IOW)),SPGZ(KOMX(IOW)),DEF(KOMX(IOW))
...
! ADDED BY LIWANG MA 7-10-2-22, FROM CODY PAPER HTTP://dx.doi.org/10.1016/j.ecolmodel.2017.02.004
      ...
      DO KKP=1,3 ! PREFERENCE LOOP
          DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              DO I=1,NN ! FORAGE LOOP
                  ...
                  MIN(1.,1.+ANTQG(KKG)-ANTQF)
                  DESX(IHD)=MAX(DESX(IHD),DESL(KKF),DESD(KKF),.01)
              END DO ! FORAGE LOOP   
              DO I=1,NN ! FORAGE LOOP
                  KKF=JE(I,ISA)
                  IF(KKF==MNC)CYCLE
      ...
      CALL SUBSUB(NN,GZSS,GZSL,GZSD,SPSL,SPSD,GZSLT,GZSDT,SPSLT,SPSDT,SPGZ)
!      SPGZ=0.														
      ...
      DO KKP=1,3 ! PREFERENCE LOOP
          DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              DO I=1,NN ! FORAGE LOOP          
                  KKF=JE(I,ISA)
                  IF(KKF==MNC)CYCLE
                  IF(TSLSD<GZLM(IHD,ISA)/HE(JT1))CYCLE
      ...
      DO KKP=1,3 ! PREFERENCE LOOP														
          DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              DO I=1,NN ! FORAGE LOOP													
                  ...
                  IF(TSLSD<GZLM(IHD,ISA)/HE(JT1))CYCLE                                                				
                  IF(YTP(KKP,IHD)<1.E-10)CYCLE
                  RTX=SPSLT(KKP,IHD,KKF)/YTP(KKP,IHD)           
!                  RTX=SPSL(KKP,IHD,KKF,ISA)/YTP(KKP,IHD)           
                  !GZSL(KKP,IHD,KKF)=MIN(SPSL(KKP,IHD,KKF),FSEL(KKP,IHD)*RTX*DMDT(IHD,IOW)) 
				  GZSLT(KKP,IHD,KKF)=MIN(SPSLT(KKP,IHD,KKF),FSEL(KKP,IHD)*RTX*DMDT(IHD,IOW)*GZDH(IHD,ISA))																						 
				  GZSL(KKP,IHD,KKF,ISA)=GZSLT(KKP,IHD,KKF)*GZSS(KKF,ISA)																						 
                  RTX=SPSDT(KKP,IHD,KKF)/YTP(KKP,IHD)           
!                  RTX=SPSD(KKP,IHD,KKF,ISA)/YTP(KKP,IHD)           
                  !GZSD(KKP,IHD,KKF)=MIN(SPSD(KKP,IHD,KKF),FSEL(KKP,IHD)*RTX*DMDT(IHD,IOW))
				  GZSDT(KKP,IHD,KKF)=MIN(SPSDT(KKP,IHD,KKF),FSEL(KKP,IHD)*RTX*DMDT(IHD,IOW)*GZDH(IHD,ISA))																						
				  GZSD(KKP,IHD,KKF,ISA)=GZSDT(KKP,IHD,KKF)*GZSS(KKF,ISA)																						 
              END DO    
          END DO
      END DO
! LIWANG MA TO ACCOUNT FOR SUB-SUBAREA GRAZING
!      CALL SUBSUB(NN,GZSS,GZSL,GZSD,SPSL,SPSD,GZSLT,GZSDT,SPSLT,SPSDT)
!     
      ! SUM GRAZING
      DO KKP=1,3 ! PREFERENCE LOOP                                 
          DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              DO I=1,NN
                  KKF=JE(I,ISA)
                  IF(KKF==MNC)CYCLE
!                  GZSX(IHD,IOW)=GZSX(IHD,IOW)+GZSL(KKP,IHD,KKF)+GZSD(KKP,IHD,KKF)
                  GZSL(KKP,IHD,KKF,ISA)=GZSLT(KKP,IHD,KKF)*GZSS(KKF,ISA)
                  GZSD(KKP,IHD,KKF,ISA)=GZSDT(KKP,IHD,KKF)*GZSS(KKF,ISA)
      ...
      DO KKP=1,3 ! PREFERENCE LOOP
          DO KHD=1,NHRD(IOW)
              ...
              DO I=1,NN ! FORAGE LOOP
                  ...
                  IF(KKF==MNC)CYCLE
!                  GZSX(IHD,IOW)=GZSX(IHD,IOW)+GZSL(KKP,IHD,KKF)+GZSD(KKP,IHD,KKF)
!                  SPSL(KKP,IHD,KKF)=SPSL(KKP,IHD,KKF)-GZSL(KKP,IHD,KKF)
!                  SPSD(KKP,IHD,KKF)=SPSD(KKP,IHD,KKF)-GZSD(KKP,IHD,KKF)
      ...
      DO KHD=1,NHRD(IOW) ! HERD LOOP
		  ...
		  DEF(IHD)=DMDT(IHD,IOW)*GZDH(IHD,ISA)-GZSX(IHD,IOW)
          ...
          DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              IF(DEF(IHD)>0.)THEN
                  DO I=1,NN ! FORAGE LOOP
                      ...
                      RTO=SPSLT(KKP,IHD,KKF)/SPGZ(IHD)
                      X1=MIN(DEF(IHD)*RTO,SPSLT(KKP,IHD,KKF))
                      AD2(IHD)=AD2(IHD)+X1
!                      GZSL(KKP,IHD,KKF)=GZSL(KKP,IHD,KKF)+X1
                      GZSLT(KKP,IHD,KKF)=GZSLT(KKP,IHD,KKF)+X1
                      GZSL(KKP,IHD,KKF,ISA)=GZSLT(KKP,IHD,KKF)*GZSS(KKF,ISA)
                      SPSLT(KKP,IHD,KKF)=SPSLT(KKP,IHD,KKF)-X1
                      SPSL(KKP,IHD,KKF,ISA)=SPSLT(KKP,IHD,KKF)*GZSS(KKF,ISA)
                      RTO=SPSDT(KKP,IHD,KKF)/SPGZ(IHD)
                      X1=MIN(DEF(IHD)*RTO,SPSDT(KKP,IHD,KKF))
                      AD2(IHD)=AD2(IHD)+X1
!                      GZSD(KKP,IHD,KKF)=GZSD(KKP,IHD,KKF)+X1
                      GZSDT(KKP,IHD,KKF)=GZSDT(KKP,IHD,KKF)+X1
                      GZSD(KKP,IHD,KKF,ISA)=GZSDT(KKP,IHD,KKF)*GZSS(KKF,ISA)
                      SPSDT(KKP,IHD,KKF)=SPSDT(KKP,IHD,KKF)-X1
                      SPSD(KKP,IHD,KKF,ISA)=SPSDT(KKP,IHD,KKF)*GZSS(KKF,ISA)
                  END DO
              END IF
          END DO
      END DO 
...
!                  ADNX(IHD)=ADNX(IHD)+CNT(KKF,ISA)*GZSL(KKP,IHD,KKF)+BN(3,KKF)*GZSD(KKP,IHD,KKF)
!                  ADP(IHD)=ADP(IHD)+CPT(KKF)*GZSL(KKP,IHD,KKF)+BP(3,KKF)*GZSD(KKP,IHD,KKF)
                  ...
                  GZSX(IHD,IOW)=GZSX(IHD,IOW)+GZSLT(KKP,IHD,KKF)+GZSDT(KKP,IHD,KKF)
                  ATDN(IHD)=ATDN(IHD)+TDNF(KKF,ISA)*GZSLT(KKP,IHD,KKF)+TDNN(KKF)*GZSDT(KKP,IHD,KKF)
                  ADNX(IHD)=ADNX(IHD)+CNT(KKF,ISA)*GZSLT(KKP,IHD,KKF)+BN(3,KKF)*GZSDT(KKP,IHD,KKF)
                  ADP(IHD)=ADP(IHD)+CPT(KKF)*GZSLT(KKP,IHD,KKF)+BP(3,KKF)*GZSDT(KKP,IHD,KKF)
                  XX=100.*HUI(KKF,ISA)
                  WC=.5*(1.-XX/(XX+EXP(SCRP(34,1)-SCRP(34,2)*XX)))+.25
!                  ADH2O(IHD)=ADH2O(IHD)+GZSL(KKP,IHD,KKF)*WC/(1.-WC)+.111*GZSD(KKP,IHD,KKF)
                  ADH2O(IHD)=ADH2O(IHD)+GZSLT(KKP,IHD,KKF)*WC/(1.-WC)+.111*GZSDT(KKP,IHD,KKF)
              END DO !FORAGE LOOP
          END DO ! HERD LOOP
      END DO ! PREFERENCE LOOP
      ...
      IF(IHAY>0)THEN 
       DO KHD=1,NHRD(IOW) ! HERD LOOP
              ...
              KKG=NGZ(IHD,ISA)
              IF(KKG==0)CYCLE
        IF(DEF(IHD)>0. .AND.Hay_total(IHD,IOW)<Hays(IHD,IOW))THEN  
      ...
      F10.1,3X,A4,27F10.3,2(I8,2X),F10.0,F10.3,F10.0,5(2X,F10.4))      
          ...
          IF(SUM>0.)TDN(IHD,ISA)=SUM/REAL(CROPN)
      END DO
      ! UPDATE PLANT STATUS.
      TOT2=0.
      TOT3=0.
      DO I=1,NN ! FORAGE LOOP
          ...
          YLN=MIN(.9*(UN1(KKF,ISA)+STDN(KKF,ISA)),YLD(KKF,ISA)*CNLV)
          YLP=MIN(.9*(UP1(KKF,ISA)+STDP(KKF,ISA)),YLD(KKF,ISA)*CPLV)
          X1=1./HE(JT1)-1.
          XZ1=YLD(KKF,ISA)*X1
          ...
          UP1(KKF,ISA)=UP1(KKF,ISA)-DFOP-YLP
