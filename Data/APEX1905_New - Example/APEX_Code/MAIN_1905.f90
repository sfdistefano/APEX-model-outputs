...
!     SIMULATION PERIOD.
      ...
      CHARACTER(10)::FMT
      CHARACTER(80)::ASOT,FGZP,FPSOD,OPSCFILE,FRFDT,RFTFILE,&
      SOILFILE,TITWP,WINDFILE,WPMFILE
...
!      OPEN (UNIT=1234,FILE='GRAZTEST.OUT')
      ! READ DIMENSIONS
    ...
    !     LINE 6
     ...
     2140 LC=0
          ...
          DO ISA=1,MSA  !SUBAREA LOOP
              ...
              IF (WSA(ISA) > 0. .AND. ISA > 1) STOP "MULTIPLE SUBAREA WITH POSITIVE AREA IN .SUB FILE"
              IF(IAZM>0)THEN
                  X1=ASIN(STP(ISA))
                  YLAZ=YLAZ/CLT
                  AZR=AZM/CLT
              ...
              DO I=1,MSL !SOIL LOOP  
                  IF (FC(I,ISA)<UW(I,ISA)) THEN
                       WRITE(*,*)'SUBAREA ',ISA,' SOIL LAYER ',I,' FIELD CAPACITY < WILTING POINT'                                           
                       WRITE(KW(36),*)'SUBAREA ',ISA,' SOIL LAYER ',I,' FIELD CAPACITY < WILTING POINT'
                       OPEN(KW50,FILE=TRIM(ADJUSTL(ASTN))//'.CMP',STATUS = 'UNKNOWN')
                       CLOSE(KW50,STATUS = 'DELETE')
                       OPEN(KW50,FILE=TRIM(ADJUSTL(ASTN))//'.CMP',STATUS = 'UNKNOWN')
                       STOP
                  END IF
                  IF (FC(I,ISA)>(1-BD(I,ISA)/2.65)) THEN
                       WRITE(*,*)'SUBAREA ',ISA,' SOIL LAYER ',I,' FIELD CAPACITY > POROSITY'                                           
                       WRITE(KW(36),*)'SUBAREA ',ISA,' SOIL LAYER ',I,' FIELD CAPACITY > POROSITY'                                           
                       OPEN(KW50,FILE=TRIM(ADJUSTL(ASTN))//'.CMP',STATUS = 'UNKNOWN')
                       CLOSE(KW50,STATUS = 'DELETE')
                       OPEN(KW50,FILE=TRIM(ADJUSTL(ASTN))//'.CMP',STATUS = 'UNKNOWN')
                       STOP
              ...
              IF (Z(1,ISA)>0.05) THEN                                                                                                       
                  ...
                  LID(2,ISA)=NBSL(ISA)                                                                                    
                  LORG(NBSL(ISA),ISA)=1                                                                                   
                  RTO=(0.05-0.01)/(Z(1,ISA)-0.01)                                                                                        
                  CALL SPLA(1,1,NBSL(ISA),RTO,1)                                                                          
                  Z(NBSL(ISA),ISA)=.05
                  ELSE
                  IF (Z(2,ISA)>0.05) THEN
                  NBSL(ISA)=NBSL(ISA)+1                                                                                   
                  DO I=NBSL(ISA),3,-1                                                                                     
                      LID(I,ISA)=LID(I-1,ISA)                                                                             
                  END DO                                                                                                  
                  LID(2,ISA)=NBSL(ISA)                                                                                    
                  LORG(NBSL(ISA),ISA)=2                                                                                   
                  RTO=(0.05-Z(1,ISA))/(Z(2,ISA)-Z(1,ISA))                                                                                        
                  CALL SPLA(2,1,NBSL(ISA),RTO,1)                                                                          
                  Z(NBSL(ISA),ISA)=.05
                  ELSE
                  Z(2,ISA)=.05
                  ENDIF
              ...
              DO J=1,NBSL(ISA)                                                                                      
                  L=LID(J,ISA)
                  IF (J==1) THEN
                     ZTH(L,ISA)=Z(L,ISA)*1000.
                  ELSE
                     ZTH(L,ISA)=(Z(L,ISA)-Z(LID(J-1,ISA),ISA))*1000.
                  ENDIF
                  XGO2(L)=CGO2(L,ISA)                                                                                     
                  XGCO2(L)=CGCO2(L,ISA)                                                                                   
                  XGN2O(L)=CGN2O(L,ISA)                                                                                   
...
!              TITOP(ISA)=OPSCFILE(1:k-4)//".CAN"
          ...
          DO I=2,MSO-1
              ...
              IF (I/=42 .AND. I/=11) WRITE(KW(I),5031)SAFILE
          END DO
          IF(KFL(45)>0)THEN
        ...
        !     IF(KFL(21)>0)CALL SOCIOA(1)
          ...
          DO    ! OUTPUT LOOP
              ...
              END IF    
              IF(KFL(11)>0) THEN
              WRITE(KW(11),732)SAFILE
              WRITE(KW(11),4086) (Z(LID(I,ISA),ISA)*100.,I=1,9),(Z(LID(I,ISA),ISA)*100.,I=1,9)
              ENDIF
...
!              (HEDS(KS(J1)),J1=1,NKS)
              ...
              DO ISA=1,MSA !SUBAREA SUMMARY LOOP
                  ...
                  DO I=1,12
                      ...
                      VAR(5,ISA)=VAR(5,ISA)+ATSW(I,ISA)
                  ...
                  IF(KFL(1)>0)THEN
                      ...
                      WRITE(KW(1),3300)'  SW',(ATSW(I,ISA),I=1,12),VAR(5,ISA),'SW  '
  ...
  586 FORMAT(1X,2I8,100E10.3)      
  587 FORMAT(8X,'WATERSHED',100E10.3)
  ...
  729 FORMAT(3X,'SA# SAID   YR  YR#',5X,'Q',5X,'SSF',5X,'PRK',4X,'QDRN',&
      ...
      --------------------------------(g/ha)----------------------------&
      --------------------|')          
  732 FORMAT(10X,A20,360X,41('='),'SOIL WATER CONTENT (CM/CM)',40('='),&
        3X,40('='),'SOIL NO3-N CONTENT (PPM)',40('='),&
        4X,40('='),'SOIL NH4-N CONTENT (PPM)',40('='))                                   
  811 FORMAT(10I8)
 ...
 2072 FORMAT(T31,'RFV',7X,'Q',5X,'SSF',5X,'YSD',10(23X,'PSRO',8X,'PSSF',&
      ...
      -----|',1X))     
 2073 FORMAT(/T5,'APEX1905(8/1/2023)',2X,3I4,2X,2(I2,':'),I2)     
 ...
 2130 FORMAT(6X,'SA#',4X,'SAID',3X,'YR',2X,'MO',2X,'DA',2X,'OPERAT',2X,'OWNER',&
              3X,'HERD#',2X,'HERDSZhd',1X,'RSTKha/hd',1X,'GZWTkg/hd',3X,'GZ_HOUR',&
               ...
               5X,'SWA15',7X,'SWA30',8X,'SN15',8X,'SN30')  !,4X,'GZLDkg/hd')
...
!2303 FORMAT(4X,A5,27X,2F5.0,I5,F10.3,F6.0,8F8.3,2F6.0,F6.1,12X,3F6.2)      
 ...
 3285 FORMAT(/6X,'ISA',6X,'ID',1X,'IDON',4X,'Y   M   D',6X,'WSAha',2X,&
      ...
      'GZSDkg/ha',5X,'DTDN',3X,'DCNCg/g',1X,'FECEkg/hd',1X,'FECNkg/hd',&
      1X,'FECPkg/hd',1X,'URINkg/hd',1X,'URNNkg/hd',1X,'URNPkg/hd',1X,&
      'H20Fkg/hd',1X,'H2OCkg/hd',1X,'MILKkg/hd',3X,'GZNBhd',2X,&
      'CALVEShd',5X,'CAWTkg',4X,'CAGZkg',6X,'CH4g',7X, &
      'SWA15',7X,'SWA30',8X,'SN15',8X,'SN30',7X,'POP')      
 ...
 3381 FORMAT('1'/T5,'APEX1905(8/1/2023)',2X,I4,2I2,2X,2(I2,':'),I2)
...
4086  FORMAT(8X,'N',6X,'ID',4X,'Y   M   D ',5X,'CPNM',7X,'WSAha',6X,' HUI',&
      8X,' LAI',8X,'  RD',8X,'  RW',8X,'BIOM',8X,' STL',8X,'CPHT',8X,' STD',&
      8X,'STDL',5X,'GZSLkg/ha',3X,'GZSDkg/ha',7X,'A_DDM',&
      7X,'PRCP', 8X,' PET', 8X, ' AET',8X,'  AT',8X,'  AE',&
      7X,'Q(mm)',6X,'AT-SP',8X,' WS ',8X,' NS ',8X,' TS ',4X,'MIN_STRESS',&
      4X,'SURF_LIT',3X,'TOTAL_LIT',3X,'LIT_TO_OM',6X,'STL_N%',6X,'STD_N%',&
      7X,'0-5CM',6X,'5-15CM',5X,'15-25CM',5X,'25-35CM',5X,'35-45CM',&
      5X,'45-55CM',5X,'55-65CM',5X,'65-75CM',5X,'75-85CM',&
      20(1X,F9.2,'CM'))
!      7X,'0-5CM',6X,'5-15CM',5X,'15-25CM',5X,'25-35CM',5X,'35-45CM',&
!      5X,'45-55CM',5X,'55-65CM',5X,'65-75CM',5X,'75-85CM',&
!      7X,'0-5CM',6X,'5-15CM',5X,'15-25CM',5X,'25-35CM',5X,'35-45CM',&
