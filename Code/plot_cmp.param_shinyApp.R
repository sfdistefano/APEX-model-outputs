  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(
        ...
        tabPanel("All APEX Runs", 
                 plotOutput("apex_plot"),
...
server <- function(input, output) {
  ...
  output$lowest_rmse_plot_inter <- renderPlot({
    ...
    ggplot(filtered_data) +
      geom_line(aes(x = date, y = S_BIOMAS, color = paste0(CPNM2, "-APEX"))) +
      geom_point(aes(x = date, y = S_BIOMAS, color = paste0(CPNM2, "-APEX"))) +
      theme_bw() +
      theme(text = element_text(size = 15, family = "serif")) +
      xlab("Year") +
      ylab("Cage Biomass (kg/ha)") +
